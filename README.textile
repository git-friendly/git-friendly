...
* *pull* will run commands like @bundle install@, @npm install@ and @bower install@ if applicable
...
<img src="http://wow.sapegin.me/image/3R3H171M1741/pull.png" alt="" title="Rolling in style" />
...
| *pull* | Stash any local changes, pull from remote using rebase, updates submodules, pop your stash, then run @bundle install@, @npm install@ or @bower install@ if necessary |
...
You can disable @bundle install@, @npm install@ and @bower install@ for the @pull@ command by defining environmental variables:
